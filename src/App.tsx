...
import { MrtLandDevelopment } from './MrtLandDevelopment';
...
const formatNtd = (value: number | undefined, language: Language) => {
  ...
  if (language === 'zh') return value >= 10_000 ? `${(value / 10_000).toLocaleString('zh-TW', { maximumFractionDigits: 0 })} 萬` : `$${value.toLocaleString('zh-TW')}`;
...
function MarketOverview({ data, language }: { data: DataBundle; language: Language }) {
  ...
  const secured = summary.movablePropertySecuredTransactionRecords; const monthlyMedianPriceRows = summary.byMonth.map((row) => ({ ...row, displayedMedian: row.saleCount >= 5 ? row.medianUnitPricePerPingNtd : undefined }));
  return <>
    <MetricStrip items={[
    ...
    <div className="chart-grid">
      ...
      <ChartSection title={t.medianUnitPriceByMonth} note={language === 'zh' ? '僅顯示每月至少 5 筆買賣紀錄的單價中位數；樣本不足的月份不繪製，以避免個別交易造成誤讀。' : 'Only months with at least five sale records display a median unit price. Smaller samples are omitted to avoid overinterpreting individual transactions.'}><ResponsiveContainer width="100%" height={280}>
        <LineChart data={monthlyMedianPriceRows}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="period" /><YAxis tickFormatter={(value) => formatPriceAxis(value, language)} /><Tooltip content={<MonthlyMedianPriceTooltip language={language} rows={monthlyMedianPriceRows} />} /><Line type="monotone" dataKey="displayedMedian" name={t.medianUnitPrice} stroke="#b24738" strokeWidth={3} dot={false} /></LineChart>
      </ResponsiveContainer></ChartSection>
...
function MonthlyMedianPriceTooltip({ active, label, language, rows }: { active?: boolean; label?: string; language: Language; rows: Array<{ period: string; saleCount: number; displayedMedian?: number }> }) {
  if (!active || !label) return null;
  const row = rows.find((item) => item.period === label);
  if (!row) return null;
  return <div className="chart-tooltip"><strong>{label}</strong><span>{language === 'zh' ? `買賣筆數：${row.saleCount}` : `Sales: ${row.saleCount}`}</span><span>{row.displayedMedian === undefined ? (language === 'zh' ? '單價中位數：樣本不足（至少 5 筆）' : 'Median unit price: insufficient sample (minimum 5)') : `${language === 'zh' ? '每坪單價中位數' : 'Median unit price'}：${formatUnitPrice(row.displayedMedian, language)}`}</span></div>;
}
...
const dashboardModules: DashboardModule[] = [
  ...
  { tab: 45, category: 'land', label: { zh: '\u6377\u904b\u571f\u5730\u958b\u767c', en: 'MRT Land Development' }, keywords: ['mrt', 'metro', 'land development', 'development status', '捷運', '土地開發'] },
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {data && <>
        ...
        {tab === 45 && <MrtLandDevelopment language={language} />}
      </>}
    </main>
